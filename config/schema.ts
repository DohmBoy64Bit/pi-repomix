...
export const OutputPatternSchema = Type.Object({
	...
	compress: Type.Optional(
		Type.Boolean({ description: "Enable compression for matched files." }),
	),
	directoryStructureOnly: Type.Optional(
		Type.Boolean({ description: "List file but omit content." }),
	),
});

// Git configuration
export const GitConfigSchema = Type.Object({
	sortByChanges: Type.Optional(
		Type.Boolean({ description: "Sort files by git change frequency." }),
	),
	sortByChangesMaxCommits: Type.Optional(
		Type.Number({ description: "Max commits to analyze for sorting." }),
	),
	includeDiffs: Type.Optional(
		Type.Boolean({ description: "Include git diffs in output." }),
	),
	includeLogs: Type.Optional(
		Type.Boolean({ description: "Include git commit logs in output." }),
	),
	includeLogsCount: Type.Optional(
		Type.Number({ description: "Number of commit logs to include." }),
	),
});
...
export const IgnoreConfigSchema = Type.Object({
	useGitignore: Type.Optional(
		Type.Boolean({ description: "Respect .gitignore file." }),
	),
	useDefaultPatterns: Type.Optional(
		Type.Boolean({ description: "Use built-in default ignore patterns." }),
	),
	customPatterns: Type.Optional(
		Type.Array(Type.String(), {
			description: "Additional custom ignore patterns.",
		}),
	),
});
...
export const SecurityConfigSchema = Type.Object({
	enableSecurityCheck: Type.Optional(
		Type.Boolean({
			description: "Enable security scanning for sensitive data.",
		}),
	),
});
...
export const TokenCountConfigSchema = Type.Optional(
	Type.Object({
		encoding: Type.Optional(
			Type.String({
				description: "Token encoding for counting (e.g., 'o200k_base').",
			}),
		),
	}),
);
...
export const InputConfigSchema = Type.Object({
	maxFileSize: Type.Optional(
		Type.Number({ description: "Maximum file size in bytes to process." }),
	),
});
...
export const RepomixConfigSchema = Type.Object({
	...
	output: Type.Optional(
		Type.Object({
			...
			parsableStyle: Type.Optional(
				Type.Boolean({ description: "Strict format compliance." }),
			),
			compress: Type.Optional(
				Type.Boolean({ description: "Enable smart code compression." }),
			),
			headerText: Type.Optional(
				Type.String({ description: "Custom header text." }),
			),
			fileSummary: Type.Optional(
				Type.Boolean({ description: "Include file summary section." }),
			),
			directoryStructure: Type.Optional(
				Type.Boolean({ description: "Include directory tree." }),
			),
			files: Type.Optional(
				Type.Boolean({ description: "Include actual file contents." }),
			),
			removeComments: Type.Optional(
				Type.Boolean({ description: "Strip code comments." }),
			),
			removeEmptyLines: Type.Optional(
				Type.Boolean({ description: "Remove empty lines." }),
			),
			topFilesLength: Type.Optional(
				Type.Number({ description: "Number of largest files in summary." }),
			),
			showLineNumbers: Type.Optional(
				Type.Boolean({ description: "Show line numbers in output." }),
			),
			patterns: Type.Optional(
				Type.Array(OutputPatternSchema, {
					description: "Per-pattern compression overrides.",
				}),
			),
			truncateBase64: Type.Optional(
				Type.Boolean({ description: "Truncate base64-encoded content." }),
			),
			copyToClipboard: Type.Optional(
				Type.Boolean({ description: "Copy output to clipboard." }),
			),
			includeEmptyDirectories: Type.Optional(
				Type.Boolean({ description: "Include empty directories in tree." }),
			),
			instructionFilePath: Type.Optional(
				Type.String({ description: "Path to custom instructions file." }),
			),
			git: Type.Optional(GitConfigSchema),
			splitOutput: Type.Optional(
				Type.Number({ description: "Split output by size in bytes." }),
			),
		}),
	),
