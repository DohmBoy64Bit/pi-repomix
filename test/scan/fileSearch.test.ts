...
function createConfig(overrides: Partial<RepomixConfigMerged> = {}): RepomixConfigMerged {
	return {
...
describe("scan/fileSearch", () => {
	...
	function createTestFiles(baseDir: string): void {
		...
		// TypeScript files
		writeFileSync(join(baseDir, "src", "app.ts"), "import { Utils } from './utils';\nexport const app = true;");
		writeFileSync(join(baseDir, "src", "utils", "helpers.ts"), "export function helper() { return 1; }");
		writeFileSync(join(baseDir, "src", "utils", "types.ts"), "export interface Config { name: string; }");
		writeFileSync(join(baseDir, "src", "components", "Button.tsx"), "export const Button = () => <button/>;");
		writeFileSync(join(baseDir, "lib", "index.ts"), "export * from '../src/app';");
		writeFileSync(join(baseDir, "lib", "helpers.ts"), "export const libHelper = () => {};\n".repeat(100));

		// JavaScript files
		writeFileSync(join(baseDir, "src", "main.js"), "const app = require('./app');\nmodule.exports = app;");
		writeFileSync(join(baseDir, "src", "utils", "legacy.js"), "function legacy() { return true; }");
		writeFileSync(join(baseDir, "lib", "bundle.js"), "// bundled output\n".repeat(50));

		// JSON files
		writeFileSync(join(baseDir, "package.json"), '{"name": "test", "version": "1.0.0"}');
		writeFileSync(join(baseDir, "tsconfig.json"), '{"compilerOptions": {"target": "ES2020"}}');
		writeFileSync(join(baseDir, "src", "config.json"), '{"debug": true}');

		// Markdown files
		writeFileSync(join(baseDir, "README.md"), "# Test Project\n\nThis is a test.");
		writeFileSync(join(baseDir, "src", "CHANGELOG.md"), "## v1.0.0\n\nInitial release.");

		// Dotfiles
		writeFileSync(join(baseDir, ".eslintrc"), '{"rules": {}}');
		writeFileSync(join(baseDir, ".babelrc"), '{"presets": ["@babel/preset-env"]}');
		writeFileSync(join(baseDir, ".gitignore"), "node_modules/\n/dist/");

		// Test files
		writeFileSync(join(baseDir, "test", "app.test.ts"), "import { app } from '../src/app';\ntest('app', () => {});");
		writeFileSync(join(baseDir, "__tests__", "helpers.test.ts"), "test('helpers', () => {});");
		...
		writeFileSync(join(baseDir, "node_modules", "fake-pkg", "index.js"), "module.exports = {};");
	}
	...
	describe("searchFiles with synthetic test directory", () => {
		...
		it("should exclude node_modules by default", async () => {
			...
			expect(result.filePaths.every((f) => !f.includes("node_modules"))).toBe(true);
		});
		...
		it("should respect custom ignore patterns", async () => {
			const result = await searchFiles(testDir, createConfig({
				include: ["**/*"],
				ignore: { useGitignore: false, useDefaultPatterns: false, customPatterns: ["**/*.md"] },
			}));
		...
		it("should respect multiple custom ignore patterns", async () => {
			const result = await searchFiles(testDir, createConfig({
				include: ["**/*"],
				ignore: { useGitignore: false, useDefaultPatterns: false, customPatterns: ["**/*.md", "**/*.json"] },
			}));
		...
		it("should handle empty directory gracefully", async () => {
			...
			const result = await searchFiles(emptyDir, createConfig({
				include: ["**/*"],
			}));
		...
		it("should handle mixed file types", async () => {
			...
			const extensions = new Set(result.filePaths.map((f) => f.split(".").pop()));
		...
		it("should handle include patterns with multiple glob patterns", async () => {
			...
			expect(result.filePaths.every((f) => f.endsWith(".ts") || f.endsWith(".tsx"))).toBe(true);
			expect(result.filePaths.length).toBeGreaterThan(0);
		});
		...
		it("should handle exclude patterns correctly", async () => {
			const result = await searchFiles(testDir, createConfig({
				include: ["**/*"],
				ignore: { useGitignore: false, useDefaultPatterns: false, customPatterns: ["**/test/**"] },
			}));
		...
		it("should handle dotfiles correctly", async () => {
			...
			const dotfiles = result.filePaths.filter((f) => f.includes(".testconfig"));
